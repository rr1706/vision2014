...
Webcam::Webcam(Webcam const&)
{
	throw std::runtime_error("copying is disabled");
}

Webcam &Webcam::operator=(Webcam const&)
{
...
void Webcam::SetStreaming(bool streaming) {
	...
	if (streaming) {
		...
		if (ret == -1 || req_bufs.count < m_nbufs) {
            perror("Unable to allocate memory-mapped buffer");
		}
		...
		for (size_t i = 0; i < m_bufs.size(); ++i) {
			...
			if (ret == -1) {
                perror("Unable to enqueue empty buffer");
			}
		}

		ret = ioctl(m_fd, VIDIOC_STREAMON, &type);
		if (ret == -1) {
            perror("Failed to enable streaming");
		}
	}
...
CameraFrame &Webcam::GetFrame(CameraFrame &frame) {
	...
	if (ret == -1) {
        perror("Unable to dequeue image frame");
	}
...
void Webcam::SetControl(int type, int value) {
	...
	if (ret == -1) {
        ...
        sprintf(str, "Unable to change control %d", type);
        perror(str);
	}
}
